...
| 5 | ~~Variable shadowing — `INTEGER`/`REAL` as user variables crashed runtime~~ — **FIXED** Sprint 19 commit `9811f5e`: `<VARS>` atom replaces `intern()`. `$$` falls back to engine ns read-only with `instance? Var` guard. t4 tests pass. | env.clj |
